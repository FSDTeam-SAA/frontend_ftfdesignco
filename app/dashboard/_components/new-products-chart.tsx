...
const API_URL = `${process.env.NEXT_PUBLIC_API_URL}/dashboard/new-products-report`;
...
const fetchNewProductsReport = async (token: string): Promise<ApiResponse> => {
...
export default function NewProductsChart() {
  ...
  const chartData = data
    ? [
        { name: "Today", value: data.today, color: COLORS[0], key: "today" },
        {
          name: "This Week",
          value: data.thisWeek,
          color: COLORS[1],
  ...
  // Get total products
  const totalProducts = data ? data.today + data.thisWeek + data.thisMonth : 0;
      ...
      <CardHeader className="pb-3">
        ...
        {/* <div className="flex gap-1 mt-3">
          {timeFilters.map((filter) => (
            ...
            >
              {filter.label}: {data ? data[filter.key as keyof ApiResponse] : 0}
            </button>
          ))}
        </div> */}
